...
export class FormInput {
    ...
    private handlerFormSubmit(e : Event){
        e.preventDefault();
        ...
        if(Array.isArray(inputs)){
            ...
            let templates: HasRender;
            templates = new Display(this.docContainer, this.hiddenDiv, this.btnPrint);
            templates.render(docData, type);
        }
    }
    ...
    private inputDatas () : [string,string,string,string,string,string,number,string,number,number,number] | void {
        ...
        if(zip > 0 && price > 0 && quantity >0 && tva > 0){
            ...
            alert("Les valeurs numeriques doivent etre > 0 !");
            return;
        }
